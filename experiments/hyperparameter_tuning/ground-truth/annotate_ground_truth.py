...
correct it, and writes a hyperparameter-tuning ground-truth JSON file whose
entries include the source `path` alongside the alignment data.
...
def main() -> None:
    ...
    while i < total:
        ...
        progress = f"[{len(existing) + 1}/{total}]  index={i}  {args.hyp_suffix}"
