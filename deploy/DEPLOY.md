...
1. **Update code** — in the existing clone:
   ...
   git pull
   ```
   No new dependencies for this change — the gate uses stdlib `hmac`.
...
3. **Install the systemd service**:
   ```bash
   uv run which streamlit          # find the binary, e.g. <repo>/.venv/bin/streamlit
...
4. **Open the firewall** for the internal subnet:
   ...
   sudo ufw allow 8501/tcp          # firewalld: firewall-cmd --add-port=8501/tcp --permanent && firewall-cmd --reload
   ```
